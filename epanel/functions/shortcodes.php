<?php
	...
	function add_shortcode_plugin( $plugin_array ) {  
		$plugin_array['column'] = THEME_URI . '/js/shortcode/column.js';  
		$plugin_array['accordion'] = THEME_URI . '/js/shortcode/accordion.js';  
		$plugin_array['toggle_box'] = THEME_URI . '/js/shortcode/toggle-box.js';  
		$plugin_array['tab'] = THEME_URI . '/js/shortcode/tab.js';  
		$plugin_array['price_item'] = THEME_URI . '/js/shortcode/price-item.js';  
		$plugin_array['testimonial'] = THEME_URI . '/js/shortcode/testimonial.js'; 
		$plugin_array['message_box'] = THEME_URI . '/js/shortcode/message-box.js';  
		$plugin_array['button'] = THEME_URI . '/js/shortcode/button.js';  
	    $plugin_array['youtube'] = THEME_URI . '/js/shortcode/youtube.js';  
	    $plugin_array['vimeo'] = THEME_URI . '/js/shortcode/vimeo.js';  
		$plugin_array['frame'] = THEME_URI . '/js/shortcode/frame.js';  
		$plugin_array['social'] = THEME_URI . '/js/shortcode/social.js';  
	    $plugin_array['list'] = THEME_URI . '/js/shortcode/list.js';  
		$plugin_array['quote'] = THEME_URI . '/js/shortcode/quote.js';  
		$plugin_array['divider'] = THEME_URI . '/js/shortcode/divider.js';  
		$plugin_array['dropcap'] = THEME_URI . '/js/shortcode/dropcap.js';  
		$plugin_array['space'] = THEME_URI . '/js/shortcode/space.js';  
	...
	function rs_accordion_shortcode( $atts, $content = null ){
		...
		$accordion = '';
		$accordion .= '<ul class="rs-accordion no-bullet">';
	...
	function rsl_toggle_box_shortcode( $atts, $content = null ){
		...
		$toggle_box = '<ul class="rs-toggle-box no-bullet">';
	...
	function rs_tab_shortcode( $atts, $content = null ){
		...
		$tab .= '<div class="rs-tab">';
			$tab .= "<ul class='rs-tab-title no-bullet'>";
				for($i=0; $i<$num; $i++){
					$active = ( $i == 0 )? 'active':'';
			...
			$tab = $tab . "<ul class='rs-tab-content no-bullet'>";
				for( $i=0; $i<$num; $i++ ){
					$active = ( $i == 0 )? 'active':'';
					$tab_id = str_replace(' ', '-', $gdl_tab_array[$i]["title"]);
	...
	function rs_button_shortcode( $atts, $content = null ){
		...
		extract( shortcode_atts(array("color" => '', "background" => '', "size" => '', "src"=> '#', 'target'=>'_self'), $atts) );	
		...
		return '<a href="' . $src . '" target="' . $target . '" class="button ' . $size . '" style="color:' . $color . '; background-color:' . $background . '; border-color:' . $button_border . '; ">' . $content . '</a>';
	}
