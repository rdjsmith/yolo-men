...
add_action( 'wp_ajax_nopriv_product_ajax', 'product_ajax_callback' );
add_action( 'wp_ajax_product_ajax', 'product_ajax_callback' );
/*
 *	@desc	Process theme contact
 */
function product_ajax_callback() {
	...
	//We shall SQL escape all inputs to avoid sql injection.
	$product_href = $wpdb->escape( $_POST['product_href'] );

	(int)$product_item_ID = rs_url_to_postid( $product_href );
	
	// return product item ID
	$data['product_id'] = $product_item_ID;
	$data['featured_image'] = '
		<figure>
			<a href="'. $product_href .'" title="View product details">
				'. get_the_post_thumbnail( $product_item_ID, 'medium' ) .'
			</a>
		</figure>
		
		<div class="alignRight">
			<!--a href="'. $product_href .'" class="button mr0">View Info</a-->
		</div>
	';
